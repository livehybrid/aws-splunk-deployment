...
#### End-to-end: who requests the cert, and how it reaches the pod

The request is declarative, nobody calls ACM PCA by hand. The chain, per Splunk
role:

1. **You declare a `Certificate`** (a cert-manager custom resource, one per role
   or a shared one), naming the DNS SANs (the operator's stable service names)
   and an `issuerRef` pointing at the `AWSPCAClusterIssuer`. It is checked into
   the `sok` layer like any other manifest.
2. **cert-manager generates the leaf key pair in-cluster** and creates a
   `CertificateRequest` (a CSR). The CA private key is never involved here, only
   the leaf.
3. **The `aws-privateca-issuer` controller is the requester.** It watches for
   `CertificateRequest`s whose issuer is its `AWSPCAClusterIssuer` and calls
   `acm-pca:IssueCertificate` on the CA, authenticated by **its IRSA role** (no
   static keys). ACM PCA signs the CSR inside its HSM and returns the
   certificate; the issuer writes it back onto the `CertificateRequest`.
4. **cert-manager assembles a Kubernetes Secret** named by the `Certificate` (say
   `splunk-cm-tls`), holding `tls.crt` (the leaf), `tls.key` (the leaf key) and
   `ca.crt` (the ACM PCA chain, the trust anchor).

So the "who requests" answer is: the `aws-privateca-issuer` controller, on behalf
of the `Certificate` you declared, using its IRSA to call ACM PCA. The leaf key
is generated by cert-manager and lives in the Secret (in-cluster, which is
normal, leaf keys belong with the workload); only the **CA signing key** stays in
the ACM PCA HSM.

**How the cert reaches the pod.** The Splunk Operator CR takes arbitrary
`volumes`, which the operator mounts into every pod of that CR, so the delivery
is three handoffs:

1. **Mount the Secret via the CR** (`crs.tf`):
   ```yaml
   volumes:
     - name: tls
       secret:
   ...
   ```
   The operator mounts it read-only at `/mnt/tls` in each pod of that CR.
2. **Point Splunk's SSL config at the mounted path**, through the CR `defaults`
   (or a small conf app). For splunkd on 8089:
   ```ini
   [sslConfig]
   serverCert          = /mnt/tls/tls.crt
   sslRootCAPath       = /mnt/tls/ca.crt
   sslVerifyServerCert = true
   ```
   and the matching `inputs.conf [SSL]` / `outputs.conf [tcpout] ... clientCert`
   for S2S. Because `ca.crt` is mounted on every pod as the trust anchor,
   `sslVerifyServerCert = true` becomes safe.
3. **Renewal is automatic, the reload is the one manual bit.** cert-manager
   renews the leaf well before expiry and rewrites the same Secret in place, so
   the projected volume in the pod updates within a minute or two. Splunk does
   not watch the file, so it needs a nudge to re-read it: a `stakater/reloader`
   annotation on the CR (restart-on-Secret-change), or let the operator roll the
   tier on the next CR change, or a scheduled per-pod `splunk restart`. In dev
   the nightly rebuild re-mints and re-reads everything for free, so only prod
   needs the reloader.

Net: **cert-manager and the issuer own request, renewal and delivery-to-Secret;
the operator owns Secret-to-pod (the `volumes` mount); Splunk conf owns
pod-to-splunkd (the SSL stanzas pointing at the mounted path).** Three clean
handoffs, and the CA key never leaves the HSM.
