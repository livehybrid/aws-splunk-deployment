...
!!! note "Reconciled to the SOK-only refactor (2026-07-15)"
    This audit ran against the pre-refactor **hybrid** codebase. The estate is now **SOK-only**: the EC2 deployment model is deleted (so the EC2 heavy-forwarders that shared the subnets are gone, and the "SOK vs park-capable EC2" cost comparison in NFR-9 no longer has an EC2 alternative to weigh against), `sok-foundation` is merged into `account` (four layers: account, iam, eks, sok), and terraform-docs is added. The sizing, QoS, IP-exhaustion, PVC and RTO findings are SOK-internal and unchanged; those referencing the EC2 estate are annotated per-finding below. **`file:line` evidence points at the pre-refactor tree unless annotated.**
...
!!! success "Remediation status"
    ...
    *SOK-only refactor 2026-07-15:* 2 findings annotated. NFR-5 (changed, the hybrid EC2 HFs no longer consume subnet IPs, easing the exhaustion picture; the CNI cap still stands) and NFR-9 (changed, the EC2 park-capable alternative it compared against is deleted, so the "SOK loses the comparison" framing is now moot; the ≈$750/mo all-in SOK figure stands). The rest are SOK-internal and unchanged.
...
- **Severity**: High
- **Status**: ✅ **DONE**, `eks/eks.tf:87-96` sets vpc-cni `configuration_values` with `WARM_IP_TARGET=4`, `MINIMUM_IP_TARGET=8`, capping per-node IP appetite. (The secondary-CIDR replan is noted as future.) [SOK-only refactor 2026-07-15: changed, the hybrid EC2 heavy-forwarders that shared the /26 subnets are deleted, freeing their IPs and easing the exhaustion pressure; the CNI cap is still the right fix and stands.]
...
- **Severity**: Medium
- **Status**: ✅ **DONE**, the [overview](../kubernetes-sok-overview.md#how-we-run-it) and the [design study cost table](../kubernetes-sok.md#cost-picture-prod-shaped-eu-west-2-july-2026) are restated to the EBS-inclusive **≈ $750/mo all-in** figure with the STOPPED-floor (~$73/mo pause model, unshipped) vs DESTROYED-floor (~$1-2/mo) distinction and park-capability as the deciding criterion. (The Cost Explorer tag report is a future add.) [SOK-only refactor 2026-07-15: changed, the EC2 park-capable alternative this weighed SOK against is deleted, so the "SOK loses the always-on-vs-parkable comparison" framing is moot, SOK-only is the estate; the ≈$750/mo all-in figure and the destroy-nightly cost guard both stand.]
...
- **Cost discipline in dev**: nightly destroy (~$3.4/day when running, ~$1-2/mo at rest), spot only where stateless, EC2/SOK exclusivity guard prevents double-running against one SmartStore bucket (guard removed with the EC2 model in the SOK-only refactor), per-layer Infracost.
