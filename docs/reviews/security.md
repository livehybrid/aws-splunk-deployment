...
!!! note "Reconciled to the SOK-only refactor (2026-07-15)"
    This audit ran against the pre-refactor **hybrid** codebase. The estate is now **SOK-only**: the EC2 model is deleted, `sok-foundation` is merged into `account` (four layers: account, iam, eks, sok), the EC2/PKI account secrets (`account/secrets.tf`, including `/monitoring/splunk/password`) are removed, the private-CA/PKI, CloudTrail/CIS metric alarms, AWS Config IAM and SNS alert modules are dropped from `account`, the repo is renamed to the public `livehybrid/aws-splunk-deployment` (Apache-2.0), and terraform-docs is added. SOK now reads env-scoped `/<env>/splunk/{password,pass4SymmKey,license}` created out of band, so SEC-1's account-layer prod-secret read is resolved. Findings materially affected carry a bracketed note on their Status line. **`file:line` evidence points at the pre-refactor tree unless annotated.**
...
!!! warning "Remediation status (reconciled 2026-07-14)"
    ...
    *SOK-only refactor 2026-07-15:* 3 findings annotated. SEC-1 (account-layer prod-secret read now resolved, secrets deleted and dev reads env-scoped; the EC2 prod-SG residuals it also cited are N/A with EC2 gone), SEC-3 (changed, dev prod-secrets-in-state concern narrows to the env-scoped SOK secret), and SEC-2 (changed, repo renamed; the broad-trust/PowerUser concern still stands). SEC-4 and SEC-8 also carry minor layer-move/scope notes.

## Findings

### [SEC-1] Dev SOK runs with prod's admin password + cluster key inside the prod VPC, cross-environment blast radius
- **Severity**: High
- **Status**: ◐ **PARTIAL**, secret reads are now var-driven (`sok/secrets.tf:22-31` via `var.sok_secret_*_id`; `dev.tfvars:55-57` points dev at `/dev/splunk/*`) and a default-deny egress NetworkPolicy is added (`sok/networkpolicy.tf`, =SEC-5) (DONE). **Still OPEN: prod SG tightening**, HF `9997-9998` from `0.0.0.0/0` (`cluster/heavy-forwarder_sg.tf:38-47`), HF `8088` from the VPC CIDR (`:61-70`), and no dev-own-VPC / private subnets. [SOK-only refactor 2026-07-15: resolved / N/A. The account-layer prod secrets (`/monitoring/splunk/password`, `/splunk/pass4SymmKey`) are deleted (`account/secrets.tf` gone) and SOK reads env-scoped `/<env>/splunk/*`, so the cross-env prod-credential read is closed; the OPEN prod-SG residuals live in the deleted `cluster` layer (EC2 model gone), so they are N/A. The dev-EKS-in-prod-VPC point stands, re-verify on the eks layer.]
- **Evidence**:
...
- **Severity**: High
- **Status**: ◐ **PARTIAL**, the OIDC trust is now scoped to master only (`iam/github_actions_terraform.tf:31-35`, `StringEquals sub = repo:...:ref:refs/heads/master`) (DONE, the biggest leg). **Still OPEN: `PowerUserAccess` remains attached (`:44-47`); no permissions boundary; the `iam:CreateRole`/`PassRole` inline grants on `splunk-*` remain (`:49-134`).** [SOK-only refactor 2026-07-15: changed, the repo is renamed to `livehybrid/aws-splunk-deployment`, so the trust `sub` now reads `repo:livehybrid/aws-splunk-deployment:...`; the broad-trust / `PowerUserAccess` / no-permissions-boundary concern is unchanged and still stands, re-verify.]
- **Evidence**:
  - `terraform/layers/iam/github_actions_terraform.tf:19-29` trust is `StringLike token…:sub = repo:livehybrid/aws-splunk-cluster:*` (any branch/tag/PR/environment; repo now renamed to `livehybrid/aws-splunk-deployment`), confirmed live (`aws iam get-role GitHubActionsTerraform`).
...
- **Severity**: Medium
- **Status**: ◐ **PARTIAL**, a `DenyNonSSLAccess` policy now applies to the `…-terraform` bucket (`account/s3.tf:139-150`, `ssl_access` default true) (DONE). **Still OPEN: `encryption_type = "AES256"` (SSE-S3, not SSE-KMS); no reader restriction; the prod admin secret still transits state.** Note the account module predates the review, so live application may itself be a gap. [SOK-only refactor 2026-07-15: changed, SOK now reads env-scoped `/<env>/splunk/*`, so it is the *dev* secret (not prod) that transits the dev state bucket, shrinking blast radius; the bucket-hardening residual (SSE-S3 vs SSE-KMS, no reader restriction) is unchanged, re-verify.]
- **Evidence**:
...
- **Severity**: Medium
- **Status**: ◐ **PARTIAL**, versioning is now enabled on smartstore (`smartstore.tf:101`) and kvbackup (`kvbackup.tf:44`), and the kvbackup lifecycle expires noncurrent versions separately (`:85-87`) (DONE for 2 of 3). **Still OPEN: the apps bucket has no versioning (`apps.tf`), and no Object Lock on kvbackup.** [SOK-only refactor 2026-07-15: changed, the three buckets moved from the deleted `sok-foundation` layer into `account` (merged); the apps-bucket-versioning and kvbackup-Object-Lock residuals are unchanged, re-verify in `account`.]
...
- **Severity**: Low
- **Status**: ○ **OPEN**, `_shared/variables.tf:35` still `default = ["0.0.0.0/0"]`; no validation added. [SOK-only refactor 2026-07-15: changed, the Splunk-web-ALB leg (`cluster/splunk_web_alb.tf`) is deleted with the EC2 model, so `trusted_cidrs` now gates only the EKS public API; the footgun and the fix are otherwise unchanged, re-verify.]
...
- **Prod indexer/manager/SH Splunk ports are SG-scoped to role SGs**, not the VPC CIDR, so dev pods cannot reach the prod indexers/CM/SH (only the LM and HF are VPC-CIDR-open, per SEC-1). EC2/SOK exclusivity guards prevent two cluster managers against one SmartStore bucket. (SOK-only refactor 2026-07-15: the EC2 role SGs and the exclusivity guards are deleted with the EC2 model, so these prod-EC2-ingress considerations no longer apply.)
